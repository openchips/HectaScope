...
/* Record to RAM (DMA RX) */
/*-----------------*/

static void litepcie_record_to_ram(const char *device_name, uint32_t size, uint8_t zero_copy)
{
    static struct litepcie_dma_ctrl dma = {.use_writer = 1};

    FILE * fo = NULL;
    int n = size / sizeof(char), i = 0;
    size_t len;
    size_t total_len = 0;
    int64_t last_time;
    ...
    if (ptr == NULL) {
        ...
        for (;;) {
            ...
            //size_t i=0; 
            char *buf_rd;
            while (buf_rd = litepcie_dma_next_read_buffer(&dma)) {
                /* Get Read buffer. */
                if (!buf_rd)
                    break;
                memcpy(ptr[i], *buf_rd, 1);
                total_len++;
                /* Stop when specified size is reached */
                if (size > 0 && total_len >= size / sizeof(char))
                    keep_running = 0;
            }
        }
...
int main(int argc, char **argv)
{
    ...
    if (!strcmp(cmd, "record_ram")) {
        ...
        litepcie_record_to_ram(litepcie_device, size, litepcie_device_zero_copy);
    /* Play cmd. */
