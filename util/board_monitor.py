...
def run_board_monitor(csr_csv, port):
    ...
    with dpg.window(label="AXAU15-ADC08DJ5200RF Control/Status"):
        ...
        dpg.add_text("")
        dpg.add_text("Core TX")
        with dpg.group(horizontal=True):
            dpg.add_text("Enable ")
            dpg.add_checkbox(tag=f"core_tx_enable")
        with dpg.group(horizontal=True):
            dpg.add_text("Ready  ")
            dpg.add_checkbox(tag=f"core_tx_ready")
    ...
    def timer_callback(refresh=1e-1, xadc_points=100):
        ...
        while dpg.is_dearpygui_running():
            ...
            # Core TX.
            dpg.set_value(f"core_tx_enable", bool(get_core_tx_enable()))
            dpg.set_value(f"core_tx_ready", bool(get_core_tx_ready()))
            # Core RX.
